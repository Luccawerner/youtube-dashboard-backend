...
| **Histórias Motivacionais** | Inspiração | `#FBBF24` | `#F59E0B` | Positivo |
| **Mistérios** | Enigma | `#6366F1` | `#4338CA` | Intrigante |
| **Pessoas Desaparecidas** | Ausência | `#64748B` | `#475569` | Melancólico |
...
export function obterCorSubnicho(subniche: string): { fundo: string; borda: string } {
  switch (subniche) {
    ...
    // ⭐ Histórias Motivacionais - Amarelo sol radiante
    case 'Histórias Motivacionais':
    case 'Historias Motivacionais': // Variação sem acento
      return { fundo: '#FBBF24', borda: '#F59E0B' };
    ...
    // 🌫️ Pessoas Desaparecidas - Cinza azulado sombrio
    case 'Pessoas Desaparecidas':
      return { fundo: '#64748B', borda: '#475569' };
...
- [ ] Stickman (azul)
- [ ] Antiguidade (dourado)
- [ ] Histórias Motivacionais (amarelo)
- [ ] Mistérios (índigo)
- [ ] Pessoas Desaparecidas (cinza azulado)
